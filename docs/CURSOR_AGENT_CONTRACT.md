...
Stale projected Mission state (for example a delivered Foundation whose Mission remained PAUSED, or scoped work that stayed ACTIVE without a recorded handoff) is corrected with `mission reconcile`, not by fabricating an ACTIVE interval or faking ordinary completion. Reconciliation event source is `USER`; GitHub is evidence, not the action. Session close is not a recorded handoff — only canonical `handoff` writes `HANDOFF_RECORDED`. See [Foundation 11](FOUNDATION_11.md).
