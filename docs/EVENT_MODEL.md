...
| `LOCAL_GIT_STATE_OBSERVED` | observation_id, clank_id, checkout_key, checkout_path, branch, detached, head, dirty, dirty_count, tracked_changes, untracked, upstream, upstream_ahead_local, upstream_behind_local, remotes `[{name,identity,redacted}]`, worktrees, alternate_checkouts, state_fingerprint, observed_at |
| `LOCAL_GIT_HARVEST_COMPLETED` | run_id, scope, started_at, finished_at, target_count, observed_changed, observed_unchanged, unavailable, skipped, errors, bounded per-target results |

`LOCAL_GIT_STATE_OBSERVED` is independently observed local checkout state (`source=LOCAL_GIT`, Mission/Session null). It is emitted on first success or semantic change only — not on identical hourly walks. Dirty filenames and unsafe remotes are never stored. `LOCAL_GIT_HARVEST_COMPLETED` (`source=SYSTEM`) records that a harvest invocation ran, including when nothing changed. See [Fleet Harvest 1](FLEET_HARVEST_1.md). Upstream ahead/behind is against the local tracking ref; remote freshness is unknown. This is not GitHub, CI, deployment, Mission completion, or health.
