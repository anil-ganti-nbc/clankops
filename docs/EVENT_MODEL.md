...
Ordinary Mission lifecycle uses `MISSION_STATE_CHANGED` and `MISSION_TRANSITIONS`. `MISSION_STATE_RECONCILED` is a later correction of **present projected knowledge** from explicit evidence. It does not rewrite or delete prior events, does not fabricate an ACTIVE interval or Session, and its `ts_utc` is when reconciliation occurred — never the evidence's own time. See [Foundation 11](FOUNDATION_11.md).
