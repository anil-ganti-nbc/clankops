...
class Store:
    ...
    def resume_mission(self, mission: str, **kwargs: Any) -> dict[str, Any]:
        ...
        result = self.transition_mission(mission, MissionState.ACTIVE, actor=actor)
        session = self.start_session(
            row["mission_id"],
            actor=actor,
            source=source,
    ...
    def handoff_mission(
        ...
        *,
        ...
        if target == MissionState.PAUSED:
            mission_row = self.pause_mission(mission, actor=actor)
        elif target == MissionState.BLOCKED:
            mission_row = self.block_mission(mission, actor=actor)
        elif target == MissionState.COMPLETED:
            mission_row = self.complete_mission(mission, actor=actor)
        elif target == MissionState.ABANDONED:
            mission_row = self.abandon_mission(mission, actor=actor)
        else:
